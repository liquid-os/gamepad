...
function playMusic(track) {
  ...
  currentMusic.volume = 0.3; // 30% volume
...
function showRiddleDisplay(question, options) {
  // Pick a reliable container (turnIndicator may be hidden by default)
  let container = document.getElementById('turnIndicator');
  if (!container) container = document.getElementById('battleScene');
  if (!container) container = document.body;
  // Ensure visible if using turnIndicator
  if (container.id === 'turnIndicator') {
  ...
  container.appendChild(riddleDisplay);
...
function updateBattleScene(data) {
  ...
  if (data.players) {
    data.players.forEach(player => {
      ...
      updateSpriteStats(player.id, player.speed, player.defense);
      updateStatusEffects(player.id, player.statusEffects || []);
      
      // Handle charging sprite state
      const sprite = document.getElementById(`sprite-${player.id}`);
      if (sprite) {
  ...
  if (data.enemies) {
    data.enemies.forEach(enemy => {
      ...
      updateSpriteStats(enemy.id, enemy.speed, enemy.defense);
...
function updateSpriteStats(spriteId, speed, defense) {
  ...
  if (statsBadge) {
    statsBadge.textContent = `🛡️${defense || 0}`;
  }
}
