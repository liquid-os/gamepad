  ...
  const CLASSES = {
    knight: {
      ...
      actions: [
        ...
        { id: 'shield_bash', desc: 'Deal $d damage and stun an enemy', usesPerCombat: 3, sound: 'sound_shieldbash', name: 'Shield Bash', damage: 10, damageType: 'physical', type: 'attack', target: 'enemy', animType: 'melee',
        ...
        { id: 'taunt', desc: 'Force enemies to attack you and reduce damage you take by 50%', name: 'Taunt', type: 'taunt', target: 'self', animType: 'support',
          applyStatus: true, duration: 2, statusEffect: 'taunt', points: 50,
          sound: 'sound_zap'
        },
      ...
      skills: [
        ...
        { id: 'execute', sound: 'sound_sword', level: 15, desc: 'Deal 30 damage, doubled against enemies below 50% health', sound: 'sound_sword', executeBonus: true, name: 'Execute', damage: 30, damageType: 'physical', stun: true, type: 'attack', target: 'enemy', animType: 'melee' },
      ],
      talents: [
    ...
    wizard: {
      ...
      actions: [
        ...
        { id: 'ice_spike', sound: 'sound_explode', usesPerCombat: 3, tint: '#4ef5e4', name: 'Ice Spike', desc: 'Deal $d damage and stun an enemy', damage: 18, damageType: 'magic', type: 'attack', target: 'enemy', animTime: 2600, animType: 'spell',
          applyStatus: true, duration: 1, statusEffect: 'stun'
         },
    ...
    cleric: {
      ...
      actions: [
        ...
        { id: 'divine_strength', sound: 'sound_magicbuff', tint: '#fffc2e', sprite: 'damagebuff', desc: 'Physical damage +10 to an ally for 2 rounds', name: 'Divine Strength', type: 'buff', target: 'ally', animType: 'support',
         ...
         { id: 'cleanse', sound: 'sound_heal', sprite: 'sunbeam', desc: 'Remove negative status effects from all allies.', name: 'Cleanse', cleanseEffect: true, type: 'special', target: 'self', animType: 'support' },
      ],
      skills: [
      ...
      talents: [
        createTalent('talent_cleric_devotion', 'Divine Devotion', 'Healing +20%, Defense +4', 3, 'rare', '✨', '✨', { healingBonus: 20, defenseBonus: 4 }, 'accessory'),
    ...
    druid: {
      ...
      actions: [
        ...
        { id: 'entangle', desc: 'Deal $d damage and stun', usesPerCombat: 4, name: 'Stonebind', damage: 5, damageType: 'magic', hitChanceModifier: 50 , stun: true, type: 'attack', target: 'enemy', animType: 'spell',
          applyStatus: true, duration: 1, statusEffect: 'stun',
          sprite: 'rock',
          sound: 'sound_explode',
          tint: '#75d962'
        ...
        { id: 'rejuvenate', desc: 'Heal ally for $h every round for 3 rounds', name: 'Rejuvenate', heal: 8, type: 'buff', target: 'ally', animType: 'support',
         ...
         { id: 'poison_1', sound: 'sound_magicimpact', desc: '$d damage for 4 rounds to an enemy', tint: '#51de35', animTime: 2200, name: 'Spider Venom', damage: 12, damageType: 'magic', type: 'attack', target: 'enemy', animType: 'spell',
          applyStatus: true, duration: 4, statusEffect: 'poison', points: 12,
          sprite: 'poison',
         },
    ...
    monk: {
      ...
      actions: [
        ...
        { id: 'meditate', sound: 'sound_heal', sprite: 'heal', name: 'Grace', desc: 'Heal $h health to an ally every round for 3 rounds', heal: 15, type: 'heal', target: 'all_allies', animType: 'support',
          applyStatus: true, duration: 3, statusEffect: 'heal', points: 15,
          tint: '#dede8c'
         },
...
const PUZZLE_TYPES = {
  button_puzzle: {
    ...
    onMaxRounds: (lobby, api) => {
      ...
      api.sendToHost('customCombatMessage', {
        text: 'The mechanism overloads! All players take damage!',
        color: '#ff4444'
      });
    }
  },
  /*riddlemaster: {
    ...
    onMaxRounds: (lobby, api) => {
      ...
      // Send custom combat message
      api.sendToHost('customCombatMessage', {
        text: 'The Riddlemaster laughs! "You are not worthy!" All players take damage!',
        color: '#ff4444'
      });
    }
  },*/
  door_puzzle: {
    ...
    onMaxRounds: (lobby, api) => {
      ...
      // Send custom combat message
      api.sendToHost('customCombatMessage', {
        text: 'The door still stands! All players take damage!',
        color: '#ff4444'
      });
    }
  }
...
module.exports = {
  ...
  onPlayerJoin(lobby, api, player) {
    ...
    // Initialize item effects for the player
    applyItemEffects(lobby, player.id);

    // Send current game state to rejoining player (include base stats for client display)
    {
      const p = lobby.state.players[player.id];
...
function generateLoot(encounterDifficulty = 'medium', lobby) {
  const lootTable = LOOT_TABLES.items;
  const numItems = Math.floor(Math.random() * 2) + 2; // 2-3 items instead of 1-2
  const loot = [];
...
function processLootRolls(lobby, itemId, rolls, api) {
  ...
  if (winner && lobby.state.players[winner]) {
    ...
    const item = ITEMS[itemId];
    if (item) {
      api.sendToPlayer(winner, 'itemAwarded', {
        itemId: itemId,
        name: item.name,
  ...
  api.sendToPlayer(winner, 'gameState', {
      phase: lobby.state.phase,
      playerData: {
...
function handleLootRoll(lobby, api, player, payload) {
  console.log('[LOOT] handleLootRoll called:', { playerId: player.id, playerName: player.username, payload });
  ...
  console.log('[LOOT] Player rolled:', { playerId: player.id, roll });
  
  // Store player's roll
  lobby.state.loot.rolls[player.id] = roll;
  lobby.state.loot.playerActions[player.id] = 'roll';
...
function handleLootPass(lobby, api, player, payload) {
  console.log('[LOOT] handleLootPass called:', { playerId: player.id, playerName: player.username, payload });
  ...
  // Store player's pass
  lobby.state.loot.playerActions[player.id] = 'pass';
...
const ITEMS = {
  ...
  health_potion: {
    id: 'health_potion',
    name: 'Health Potion',
    description: 'Action: Heals 30 health to self (1 use)',
    type: 'consumable',
    rarity: 'uncommon',
    ...
    on_use: {
      id: 'health_potion',
      name: 'Health Potion',
      description: 'Heals 30 health to self',
      type: 'heal',
      heal: 30,
      damageType: 'magic',
      target: 'self',
...
function applyStatusEffect(target, effectId, duration, appliedBy, points=0, lobby=null) {
  ...
  if (existingEffect) {
    ...
    existingEffect.points = effectId == 'poison' ? existingEffect.points + points : Math.max(existingEffect.points, points);
...
function getEffectiveDefense(target) {
  // Calculate defense including status effect bonuses
...
function getStatusEffectDefense(target) {
  // Get only defense from status effects
  let statusDefense = 0;
...
const STATUS_EFFECT_NAMES = ['poison', 'stun', 'slow', 'curse', 'vulnerable', 'rage', 'defense_buff', 'physical_buff', 'magic_buff'];
...
function generateItemDescription(effects, proc) {
  ...
  if (proc) {
    const procChance = proc.procChance === 100 ? '' : `${proc.procChance}% chance to `;
    const procDescription = generateProcDescription(proc);
    effectDescriptions.push(`${procChance}${procDescription}`);
  }
...
function handleClassSelection(lobby, api, player, payload) {
  ...
  if (lobby.state.players[player.id].class) {
    const oldClass = lobby.state.players[player.id].class;
    delete lobby.state.selectedClasses[oldClass];
  }

  // Assign class to player
  const classData = CLASSES[className];
  lobby.state.players[player.id].class = className;
  lobby.state.players[player.id].maxHealth = classData.baseHealth;
  lobby.state.players[player.id].health = classData.baseHealth;
  lobby.state.players[player.id].defense = classData.baseDefense;
  lobby.state.players[player.id].actions = [...classData.actions];
  lobby.state.players[player.id].learnedSkills = [];  // Track learned skills
  
  // Apply item effects to ensure they're calculated
  applyItemEffects(lobby, player.id);
  
  lobby.state.selectedClasses[className] = player.id;
  ...
  api.sendToHost('hostGameUpdate', {
    ...
    players: Object.values(lobby.state.players).map(p => ({
      username: p.username,
      class: p.class ? CLASSES[p.class].name : 'Selecting...'
    }))
  });
...
function handleCampActionSelection(lobby, api, player, payload) {
  ...
  // Store the action
  lobby.state.camp.pendingActions[player.id] = actionId;
...
function handleSkillSelection(lobby, api, player, payload) {
  ...
  if (!playerData || playerData.health <= 0) {
    console.log(`[SKILLS] ERROR: Player not found or dead`);
    return;
  }
...
function handleTalentSelection(lobby, api, player, payload) {
  ...
  if (!playerData) {
    console.error(`[TALENTS] Player ${player.id} not found`);
    return;
  }
...
function createEnemyInstance(enemyTemplate, encounterNumber, playerCount) {
  const damageMultiplier = 1 + (0.05 + (0.05 * playerCount)) * (encounterNumber - 1);
...
function rollInitiative(lobby, api) {
  ...
  Object.values(lobby.state.players).forEach(player => {
    if (player.health > 0) {
      ...
      const roll = Math.floor(Math.random() * 20) + 1 + getTotalStatusEffectPoints(player, 'speed') - getTotalStatusEffectPoints(player, 'slow') + speedRollModifier;
      player.speed = roll;
      turnOrder.push({
        type: 'player',
        id: player.id,
  ...
  lobby.state.combat.enemies.forEach(enemy => {
    if (enemy.currentHealth > 0 && (enemy.damage > 0 || enemy.forceInitiative)) {
      ...
      roll += getTotalStatusEffectPoints(enemy, 'speed') - getTotalStatusEffectPoints(enemy, 'slow');
      enemy.speed = roll;
  ...
  Object.values(lobby.state.players).forEach(player => {
    if (player.health > 0 && player.class) {
      ...
      lobby.state.combat.playerActionOptions[player.id] = [...actionOptions];
      ...
      api.sendToPlayer(player.id, 'actionOptions', {
        ...
        yourTurn: turnOrder.findIndex(t => t.id === player.id),
...
function scaleActionForPlayer(action, player, lobby) {
  // Calculate player's item effects
  const itemEffects = applyItemEffects(lobby, player.id);
  const statePlayer = (lobby && lobby.state && lobby.state.players && lobby.state.players[player.id]) || player;
...
function handleActionSelection(lobby, api, player, payload) {
  ...
  if (!lobby.state.combat.pendingActions[player.id]) {
    ...
    lobby.state.combat.pendingActions[player.id] = {
      actions: [],
  ...
  const pending = lobby.state.combat.pendingActions[player.id];
  ...
  // If action needs a target, send target options
...
function handleTargetSelection(lobby, api, player, payload) {
  ...
  if (pending.actions.length < pending.totalActions) {
    ...
    try {
      ...
      let options = lobby.state.combat.playerActionOptions[player.id] || [];
      ...
      lobby.state.combat.playerActionOptions[player.id] = options;
    } catch (e) {
...
function showNextActionOptions(lobby, api, player) {
  ...
  let actionOptions = lobby.state.combat.playerActionOptions[player.id] || [];
...
function checkAllActionsSelected(lobby, api) {
  ...
  const allSelected = alivePlayers.every(p => {
    const pending = lobby.state.combat.pendingActions[p.id];
...
async function resolveCombatRound(lobby, api) {
  ...
  for (let i = 0; i < lobby.state.combat.turnOrder.length; i++) {
    ...
    if (turn.type === 'player') {
      const player = lobby.state.players[turn.id];
      const pending = lobby.state.combat.pendingActions[turn.id];
...
function applyAttackToTarget(action, attacker, target, attackerItemEffects, attackerStatusEffects, log, lobby) {
  ...
  // Apply damage with taunt reduction and thorns reflection
  const actualDamage = applyDamageToTarget(target, finalDamage, log, attacker);
  ...
  return { hit, crit, damage: finalDamage };
}
...
function executePlayerAction(lobby, api, player, action, targetId, animationData) {
  ...
  if(action.target == 'self'){
    ...
    if(targetId) {
      if (animationData) {
        animationData.targetId = targetId;
      }
      if(action.target == 'enemy'){
        targets.push(lobby.state.combat.enemies.find(e => e.id === targetId));
      }
      if(action.target == 'ally')
        targets.push(targetId ? lobby.state.players[targetId] : player);
    }
  }
  ...
  if (action.type === 'buff') {
    ...
    if (action.target === 'self') {
      ...
      // Single ally target
      const target = Object.values(lobby.state.players).find(p => p.id === targetId);
...
function endCombat(lobby, api, result) {
  if (result === 'victory') {
    ...
    setTimeout(() => {
      ...
      const loot = generateLoot('medium', lobby); // Could be based on enemy difficulty
