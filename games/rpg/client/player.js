...
socket.on('gameState', (data) => {
  ...
  if (data.phase === 'class_selection') {
    ...
    // Show Pick a Path phase
    console.log('[CLIENT] Reconnected to pick_a_path phase');
    if (data.pickAPath) {
      showPickAPathPhase(data.pickAPath);
    }
    ...
    // If combat phase, request action options if we're in selecting phase
    if (data.phase === 'combat' && data.combat) {
      console.log('[CLIENT] Reconnected to combat phase');
      // Request action options from server (in case we missed them)
      setTimeout(() => {
        console.log('[CLIENT] Requesting action options from server');
        socket.emit('action', {
          code: lobbyCode,
          data: {
...
socket.on('pickAPathStarted', (data) => {
  ...
  gameState.phase = 'pick_a_path';
  
  showPickAPathPhase(data);
});

socket.on('pathChosen', (data) => {
  console.log('Path chosen:', data);
  const pathName = data.choice === 'left' ? 'Go Left ⬅️' : 'Go Right ➡️';
  document.getElementById('votedPathName').textContent = pathName;
  document.getElementById('pathVoteConfirmed').style.display = 'block';
  document.getElementById('leftPathButton').disabled = true;
  document.getElementById('rightPathButton').disabled = true;
});
...
socket.on('actionOptions', (data) => {
  console.log('[CLIENT] Action options received:', data);
  console.log('[CLIENT] Number of actions:', data.actions ? data.actions.length : 0);
  console.log('[CLIENT] Number of consumables:', data.consumables ? data.consumables.length : 0);
  console.log('[CLIENT] Current phase:', gameState.phase);
  // Auto-switch to combat tab
  switchTab(0);
  showActionSelection(data.actions, data.consumables || [], data.actionIndex, data.totalActions, data.selectedActionIds || []);
  console.log('[CLIENT] Action selection UI shown');
});
...
function showPickAPathPhase(data) {
  // Hide all other phases
  classSelection.style.display = 'none';
  combatPhase.style.display = 'none';
  gameEnd.style.display = 'none';
  document.getElementById('skillLearning').style.display = 'none';
  document.getElementById('talentLearning').style.display = 'none';
  
  // Show Pick a Path phase
  const pickAPathPhase = document.getElementById('pickAPath');
  pickAPathPhase.style.display = 'block';
  ...
  // Reset vote confirmed state
  document.getElementById('pathVoteConfirmed').style.display = 'none';
  document.getElementById('leftPathButton').disabled = false;
  document.getElementById('rightPathButton').disabled = false;
  
  // Add button event listeners
  document.getElementById('leftPathButton').onclick = () => voteForPath('left');
  document.getElementById('rightPathButton').onclick = () => voteForPath('right');
}

function voteForPath(choice) {
  console.log(`[PICK_A_PATH] Voting for ${choice} path`);
  console.log(`[PICK_A_PATH] Emitting action with:`, { action: 'pathVote', payload: { choice } });
  ...
  console.log(`[PICK_A_PATH] Vote emitted successfully`);
  
  // Show confirmation
  const pathName = choice === 'left' ? 'Go Left ⬅️' : 'Go Right ➡️';
  document.getElementById('votedPathName').textContent = pathName;
  document.getElementById('pathVoteConfirmed').style.display = 'block';
...
function showActionSelection(actions, consumables = [], actionIndex = 0, totalActions = 1, selectedActionIds = []) {
  hideTargetSelection();
  hideWaitingState();
  ...
  actions.forEach(action => {
    ...
    // Add consumable count indicator
    if (action.isConsumable && action.consumableCount !== undefined) {
      const countColor = action.consumableCount > 2 ? '#22c55e' : action.consumableCount === 2 ? '#fbbf24' : '#ef4444';
      content += `<div style="font-size: 0.75em; color: ${countColor}; font-weight: bold; margin-bottom: 3px;">🧪 ${action.consumableCount} in inventory</div>`;
    }
  ...
  // Add consumables section if there are any
  if (consumables && consumables.length > 0) {
    // Add separator line
    const separator = document.createElement('div');
    separator.style.cssText = 'margin: 20px 0; border-top: 2px solid rgba(255, 255, 255, 0.3);';
    actionButtons.appendChild(separator);
    
    // Add consumables header
    const consumablesHeader = document.createElement('div');
    consumablesHeader.style.cssText = 'text-align: center; margin: 15px 0 10px 0; font-size: 1em; font-weight: bold; color: #a78bfa; text-transform: uppercase; letter-spacing: 1px;';
    consumablesHeader.textContent = '💊 Consumables';
    actionButtons.appendChild(consumablesHeader);
    
    // Add consumable action buttons
    consumables.forEach(action => {
      const btn = document.createElement('button');
      const isSelected = selectedActionIds.includes(action.id);
      ...
      // Add consumable count indicator
      if (action.consumableCount !== undefined) {
        const countColor = action.consumableCount > 2 ? '#22c55e' : action.consumableCount === 2 ? '#fbbf24' : '#ef4444';
        content += `<div style="font-size: 0.75em; color: ${countColor}; font-weight: bold; margin-bottom: 3px;">🧪 ${action.consumableCount} in inventory</div>`;
      }
...
// NEW: Helper functions to show/hide sticky tab bar
function showStickyTabBar() {
  const stickyBar = document.getElementById('stickyTabBar');
  const app = document.getElementById('app');
  if (stickyBar) {
    stickyBar.style.display = 'flex';
    app.classList.add('with-sticky-tabs');
  }
}

function hideStickyTabBar() {
  const stickyBar = document.getElementById('stickyTabBar');
  const app = document.getElementById('app');
  if (stickyBar) {
    stickyBar.style.display = 'none';
    app.classList.remove('with-sticky-tabs');
  }
}
...
function switchTab(tabIndex) {
  ...
  mobileTabButtons.forEach((btn, idx) => btn.classList.toggle('active', idx === tabIndex));
  
  // NEW: Also update sticky tab bar
  const stickyTabButtons = document.querySelectorAll('.sticky-tab-btn');
  stickyTabButtons.forEach((btn, idx) => btn.classList.toggle('active', idx === tabIndex));
...
// NEW: Sticky top tab navigation
const stickyTabBar = document.getElementById('stickyTabBar');
if (stickyTabBar) {
  const stickyTabButtons = stickyTabBar.querySelectorAll('.sticky-tab-btn');
  stickyTabButtons.forEach((btn, index) => {
...
if (mobileTabNav) {
  const mobileTabButtons = mobileTabNav.querySelectorAll('.mobile-tab-btn');
  mobileTabButtons.forEach((btn, index) => {
    btn.addEventListener('click', () => {
      switchTab(index);
    });
  });
